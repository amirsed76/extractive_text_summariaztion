...
def make_dataloader(data_file, vocab, hps, filter_word, w2s_path, data_type="train", graphs_dir=None, from_index=0,
                    to_index=None,
                    shuffle=False):
    if hps.use_cache_graph:
        dataset = CachedSummarizationDataSet(hps=hps, vocab=vocab, data_path=data_file,
                                             from_index=from_index, to_index=to_index, data_type=data_type)
    else:
